...
import me.xiaosheng.word2vec.Test;
...
//    learn.saveModel(new File("C:\\Users\\jayjosby.liu@sap.com\\Desktop\\Temp\\yuliao\\model\\javaVector"));
  ...
  public void initMatA(){
    Random random =new Random();
    // 00 self loop is 0.75~1
    String method = "self-loop";
    if(method.equals("self-loop")){
      double v11=random.nextInt(20);
      double v22=random.nextInt(20);
      double v00=100-v11-v22;
      A= new double[][]{{v00/100, 0, 0},
    ...
    }else if(method.equals("full-loop")){
      double v00=100-random.nextInt(10);
      double v01=random.nextInt(100),
              v02=random.nextInt(100),
              v11=random.nextInt(100),
              v12=random.nextInt(100),
              v22=random.nextInt(100);
      double sum = v01+v02+v11+v12+v22;
      v01 = Double.parseDouble(String.format("%.2f",v01*(100-v00)/sum));
      v02 = Double.parseDouble(String.format("%.2f",v02*(100-v00)/sum));
      v11 = Double.parseDouble(String.format("%.2f",v11*(100-v00)/sum));
      v12 = Double.parseDouble(String.format("%.2f",v12*(100-v00)/sum));
      v22 = Double.parseDouble(String.format("%.2f",v22*(100-v00)/sum));

      A= new double[][]{{v00, v01, v02},
              {v01, v11, v12},
              {v02, v12, v22}
      };
    }
  ...
  public void doLearn(Learn learn) throws IOException {
    learn.initMap();
    //随机初始化关系矩阵A
  ...
  public boolean doCompare(Learn learn) throws IOException {
    ...
    //汉语
    //String []datasets = {"CN-WS-240","CN-WS-297"};
    //计算出当前模型对应的所有数据集表现
    for(String dataset:datasets){
      ...
      List<Double> score_df = new ArrayList<>();
      ...
      while((temp = br.readLine()) != null) {
        ...
        // df score 初始化, 每次只向score-fd里添加了一个double
        BufferedReader br_df = new BufferedReader(new FileReader("C:\\Users\\jayjosby.liu@sap.com\\Desktop\\Temp\\yuliao\\ws\\result\\score_df.txt"));
        String l = br_df.readLine();
        int len = score_df.size();
        while(l!=null){
          String ls[] = l.split(" ");
          if(ls[0]==word1||ls[1]==word2){
            System.out.println(ls[0]+" "+ls[1]+"-------------------------");
          }
          if(ls[0]==word1&&ls[1]==word2){
            score_df.add(Double.valueOf(ls[2]));
          }
          l = br_df.readLine();
        }
        if(score_df.size()==len){
          score_df.add(0.0);
        }
      ...
      double pearson_df=PearsonCorrelation.getPearsonCorrelationScore(score_labeled, score_df);
      ...
      if(pearson_test>pearson_w2v){
        ...
        learn.doSave_worse(dataset, pearson_test, pearson_w2v);
      }
    }

    sentenceClassificationTask();
    sentenceSimilarityTask();
    //加句子相似度计算
    return  true;
  }

  public List<Double> getScore_fd(){
    return null;
  }
  //词相似度计算结果保存
  ...
  public void doSave_worse(String dataset, double pearson_test, double pearson_w2v) throws IOException {
    // 保存关系矩阵+相关性系数*2+score_test
    System.out.println("doSave- 词相似度task");
    BufferedWriter bw = new BufferedWriter(new FileWriter(
            "C:\\Users\\jayjosby.liu@sap.com\\Desktop\\Temp\\yuliao\\ws\\result\\mark_worse_"+dataset+".txt",true));
    // 保存关系矩阵
    bw.newLine();
    for(int i=0;i<A.length;i++){
      for(int j=0;j<A[0].length;j++){
  ...
  public void sentenceClassificationTask() throws IOException {
    String []datasets = {"Yelp","IMDB","scale"};
    // 训练贝叶斯分类器需要的数据集
    ...
    for(String dataset:datasets){
      ...
      while (line!=null){
        //txt格式 label‘\t’she is a ....
        //样本格式s= label,1,2,3,4.....
        seq1 = line[1];
        ArrayList<Double> sl1 = getSentenceVector(seq1);
        String s = line[0];
  ...
  public ArrayList<Double> getSentenceVector(String str) {
  ...
  public void sentenceSimilarityTask() throws IOException {
    ...
    for(String dataset:datasets){
      ...
      Double pearson_w2v = PearsonCorrelation.getPearsonCorrelationScore(list_score_w2v,list_score_labeled);
      System.out.println("doCompare- pearson_test :"+pearson_test+"  pearson_w2v :"+pearson_w2v);
      if(pearson_test>pearson_w2v){
        //doSave
        doSave(dataset, pearson_test, pearson_w2v);
      }else{
        doSave_worse(dataset,pearson_test, pearson_w2v);
      }
    }
  }
