...
public class Bayes {
    ...
    public static boolean loadData(List<String> list) {//加载测试的数据文件，是list<String> String label,1,2,3,4 注意长度
        try {
            for (int i=0; i<list.size(); i++) {
                String str = list.get(i);//将文件的每一行存到str的临时变量中
    ...
    public static void pretreatment(Vector<String> indata) {   //数据预处理，将原始数据中的每一个属性值提取出来存放到Vector<double[]>  data中
        ...
        while (i < indata.size()) {//取出indata中的每一行值
            ...
            int[] tem = new int[sourceStrArray.length-1];
            switch (sourceStrArray[0]) {
                case "R": {
                    for (int j = 1; j < sourceStrArray.length; j++) {
                        tem[j - 1] = (int)Double.parseDouble(sourceStrArray[j]);
                    }
                    catagory_R.add(tem);
                    break;
                }
